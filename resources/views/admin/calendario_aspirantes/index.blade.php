<x-admin-layout>
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-extrabold text-gray-800">Calendario Académico de Aspirantes</h1>
        <form method="POST" action="{{ route('admin.calendario_aspirantes.store') }}" class="flex gap-2">
            @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="md:col-span-3 bg-white rounded-lg shadow p-4">
            <div class="flex justify-between items-center mb-4">
                <a href="{{ route('admin.calendario_aspirantes.index',['mes'=>$inicio->copy()->subMonth()->month,'anio'=>$inicio->copy()->subMonth()->year]) }}">⬅</a>
                <h2 class="text-lg font-semibold">
                    {{ $inicio->translatedFormat('F Y') }}
                </h2>
                <a href="{{ route('admin.calendario_aspirantes.index',['mes'=>$inicio->copy()->addMonth()->month,'anio'=>$inicio->copy()->addMonth()->year]) }}">➡</a>
            </div>
                            ...
                            @for($d = 0; $d < 7; $d++)
                                ...
                                <td class="h-20 border align-top p-1 {{ $current->month != $mes ? 'bg-gray-50 text-gray-400' : '' }}">
                                    ...
                                    @foreach($eventoDelDia as $ev)
                                        <div class="mt-1 text-xs bg-green-100 text-green-700 rounded px-1">
                                            {{ $ev->titulo }}
                                        </div>
                                    @endforeach
                                </td>
